...
// Generate static paths for all projects — both MDX files and config entries
export async function generateStaticParams() {
  const mdxSlugs = getProjectCaseStudySlugs();
  const configSlugs = projects
    .filter((p) => p.details && p.projectDetailsPageSlug)
    .map((p) => p.projectDetailsPageSlug.replace('/projects/', ''));

  const allSlugs = [...new Set([...mdxSlugs, ...configSlugs])];
