...
/**
 * Convert a GitHub repo URL to the raw README URL (tries HEAD ref so it works
 * regardless of whether the default branch is main or master).
 */
function getGithubReadmeUrl(githubUrl: string): string {
...
/**
 * Get a project case study, trying the local MDX file first and falling back
 * to fetching the GitHub README when no MDX file exists or it has invalid
 * frontmatter. Frontmatter for the fallback is derived from the Projects config.
 */
export async function getProjectCaseStudy(
  slug: string,
): Promise<ProjectCaseStudy | null> {
  // 1. Try local MDX file
  const mdxResult = getProjectCaseStudyBySlug(slug);
  if (mdxResult) return mdxResult;

  // 2. Find the project in the config
  const project = projects.find(
    (p) => p.projectDetailsPageSlug === `/projects/${slug}`,
  );
  if (!project || !project.github) return null;

  // 3. Fetch the GitHub README
  try {
    const readmeUrl = getGithubReadmeUrl(project.github);
    ...
    if (!response.ok) {
      console.error(
        `Failed to fetch README for ${slug}: HTTP ${response.status}`,
      );
      return null;
    }
    ...
    console.error(`Error fetching GitHub README for ${slug}:`, error);
    return null;
  }
}
