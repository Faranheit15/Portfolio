...
const Index = () => {
  ...
  const skills = [
    ...
    'Firebase', 'AWS', 'OpenAI', 'Generative AI'
  ];

  const featuredProjects = [
    {
      title: 'Invoicey',
      description: 'Full-featured invoice management system with real-time collaboration',
      tech: ['Next.js', 'MongoDB', 'Firebase'],
      link: '/projects'
    },
    {
      title: 'Netflix-GPT',
      description: 'AI-powered movie recommendation platform with natural language search',
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="container-custom relative z-10">
          <div className="text-center space-y-8">
            {/* Profile Image */}
            <div className={`mx-auto w-32 h-32 rounded-full bg-gradient-to-br from-primary to-claude-accent p-1 transition-all duration-1000 ${isVisible ? 'scale-100 opacity-100' : 'scale-75 opacity-0'}`}>
              <div className="w-full h-full rounded-full bg-background flex items-center justify-center">
                <span className="text-3xl font-serif font-bold text-primary">FM</span>
              </div>
            </div>
            ...
            {/* Tagline */}
            <div className={`max-w-3xl mx-auto transition-all duration-1000 delay-500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
              <p className="text-lg sm:text-xl text-muted-foreground leading-relaxed">
                Turning concepts into{' '}
                <span className="gradient-text font-semibold">intelligent, scalable</span>{' '}
                user experiences with 3.5+ years of expertise in React, Next.js, and AI integration.
              </p>
            </div>
            ...
            {/* Social Links */}
            <div className={`flex justify-center space-x-6 transition-all duration-1000 delay-900 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
              {[
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {skills.map((skill, index) => (
                <Badge 
                  key={skill}
                  variant="secondary"
                  className={`text-sm px-4 py-2 hover-lift transition-all duration-300 delay-${index * 100}`}
                >
                  {skill}
                </Badge>
              ))}
        ...
        <div className="container-custom">
          <div className="space-y-12">
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
                A selection of my recent work showcasing technical skills and creative problem-solving
              </p>
            </div>

            <div className="grid md:grid-cols-2 gap-8">
              {featuredProjects.map((project, index) => (
                <Card key={project.title} className="hover-glow transition-all duration-300 border-0 shadow-lg">
      ...
      <section className="py-20 bg-primary text-primary-foreground">
        <div className="container-custom text-center space-y-8">
          ...
          <p className="text-xl opacity-90 max-w-2xl mx-auto">
            Let's collaborate and turn your ideas into powerful, scalable web solutions.
          </p>
          <Button 
            asChild
            size="lg"
